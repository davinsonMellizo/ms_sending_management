...
public class AlertClientUseCase {
    ...
    public Mono<AlertClient> saveAlertClient(List<AlertClient> alertClients) {
        return alertClientGateway.save(alertClients)
                .flatMap(alertClientSaved -> newnessUseCase.saveNewness(alertClients, CREATE_CLIENT_ALERT));
    }
    ...
    private Mono<AlertClient> validateCreation(Tuple2<List<String>, AlertClient> data) {
        return Mono.just(data.getT2())
                ...
                .flatMap(alertClient -> newnessUseCase.saveNewness(List.of(alertClient), UPDATE_CLIENT_ALERT))
                .switchIfEmpty(saveAlertClient(List.of(data.getT2())));
    }
    ...
    public Mono<AlertClient> deleteAlertClient(AlertClient alertClient) {
        return alertClientGateway.findAlertClient(alertClient)
                ...
                .flatMap(alertClientDeleted -> newnessUseCase.saveNewness(List.of(alertClientDeleted), DELETE_CLIENT_ALERT))
                .switchIfEmpty(Mono.error(new BusinessException(ALERT_CLIENT_NOT_FOUND)));
    }
